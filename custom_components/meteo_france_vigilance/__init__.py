...
from .const import CONF_BASINS, CONF_DEPARTMENTS, DOMAIN, SERVICE_REFRESH
from .coordinator import VigilanceCoordinator
from .coordinator_om import VigilanceOmCoordinator
...
class VigilanceRuntime:
    """Les sources de l'entrée : la métropole, l'outre-mer, ou les deux.

    Deux coordinateurs plutôt qu'un seul qui agrégerait : les services sont
    distincts, et une panne outre-mer ne doit pas rendre les départements
    métropolitains indisponibles — ni l'inverse. Chacun est absent si rien ne
    le concerne dans la configuration.
    """

    metropole: VigilanceCoordinator | None = None
    outre_mer: VigilanceOmCoordinator | None = None

    @property
    def coordinators(self) -> list[VigilanceCoordinator | VigilanceOmCoordinator]:
        """Les sources effectivement en service."""
        return [c for c in (self.metropole, self.outre_mer) if c is not None]


type VigilanceConfigEntry = ConfigEntry[VigilanceRuntime]
    ...
    async def _async_refresh(call: ServiceCall) -> None:
        ...
        for entry in entries:
            for coordinator in entry.runtime_data.coordinators:
                ...
                # volontaire, il ne passe pas par l'anti-rebond.
...
async def async_setup_entry(hass: HomeAssistant, entry: VigilanceConfigEntry) -> bool:
    ...
    if options.get(CONF_DEPARTMENTS):
        runtime.metropole = VigilanceCoordinator(hass, entry)
        await runtime.metropole.async_config_entry_first_refresh()

    if options.get(CONF_BASINS):
        runtime.outre_mer = VigilanceOmCoordinator(hass, entry)
        await runtime.outre_mer.async_config_entry_first_refresh()
